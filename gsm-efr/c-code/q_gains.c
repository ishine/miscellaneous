...
)
{
    ...
    gain_q14 = efr_shl (*gain, 2);

    err_min = efr_abs_s (efr_sub (gain_q14, qua_gain_pitch[0]));
    index = 0;                                  move16 (); 

    for (i = 1; i < NB_QUA_PITCH; i++)
    {
        err = efr_abs_s (efr_sub (gain_q14, qua_gain_pitch[i]));

        test (); 
        if (efr_sub (err, err_min) < 0)
        {
            err_min = err;                      move16 (); 
            index = i;                          move16 (); 
        }
...
)
{
    ...
    if ((txdtx_ctrl & TX_SP_FLAG) != 0)
    {
        ...
        err_min = efr_abs_s (efr_sub (*gain, efr_mult (gcode0, qua_gain_code[0])));
        index = 0;              move16 (); 

        for (i = 1; i < NB_QUA_CODE; i++)
        {
            err = efr_abs_s (efr_sub (*gain, efr_mult (gcode0, qua_gain_code[i])));
        ...
        *gain = efr_mult (gcode0, qua_gain_code[index]);
                                                move16 (); 
    ...
    else
    {
        ...
        *gain = CN_excitation_gain;             move16 (); 
        ...
        if ((txdtx_ctrl & TX_SID_UPDATE) != 0)
        {
            ...
            err_min = efr_abs_s (efr_sub (aver_gain, 
                                  efr_mult (gcode0_CN, qua_gain_code[0])));
            index = 0;                          move16 (); 

            for (i = 1; i < NB_QUA_CODE; i++)
            {
                err = efr_abs_s (efr_sub (aver_gain, 
                                  efr_mult (gcode0_CN, qua_gain_code[i])));
