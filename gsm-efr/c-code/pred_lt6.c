...
)
{
    ...
    frac = efr_negate (frac);
    test (); 
    if (frac < 0)
    {
        frac = efr_add (frac, UP_SAMP);
        x0--;
    }
    for (j = 0; j < L_subfr; j++)
    {
        ...
        x2 = x0;                move16 (); 
        c1 = &inter_6[frac];
        c2 = &inter_6[efr_sub (UP_SAMP, frac)];

        s = 0;                  move32 (); 
        for (i = 0, k = 0; i < L_INTERPOL; i++, k += UP_SAMP)
        {
            s = efr_L_mac (s, x1[-i], c1[k]);
            s = efr_L_mac (s, x2[i], c2[k]);
        }

        exc[j] = gsm_efr_round (s);     move16 (); 
    }
