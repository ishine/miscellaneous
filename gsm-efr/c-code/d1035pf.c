...
)
{
    ...
    for (j = 0; j < NB_TRACK; j++)
    {
        ...
        i = efr_extract_l (efr_L_shr (efr_L_mult (i, 5), 1));
        pos1 = efr_add (i, j); /* position of pulse "j" */

        i = efr_shr (tmp, 3) & 1;                           logic16 (); 
        test (); 
        if (i == 0)
        {
        ...
        i = index[efr_add (j, 5)] & 7;                      logic16 (); 
        i = dgray[i];                                   move16 (); 
        i = efr_extract_l (efr_L_shr (efr_L_mult (i, 5), 1));

        pos2 = efr_add (i, j);      /* position of pulse "j+5" */

        test (); 
        if (efr_sub (pos2, pos1) < 0)
        {
            sign = efr_negate (sign);
        }
        cod[pos2] = efr_add (cod[pos2], sign);              move16 (); 
    }
