...
)
{
    ...
    if ((txdtx_ctrl & TX_SP_FLAG) != 0)
    {
        for (i = 0; i < M; i++)
        {
            lsf_p[i] = efr_add (mean_lsf[i], efr_mult (past_r2_q[i], PRED_FAC));
                                                        move16 (); 
            lsf_r1[i] = efr_sub (lsf1[i], lsf_p[i]);        move16 (); 
            lsf_r2[i] = efr_sub (lsf2[i], lsf_p[i]);        move16 (); 
        }
    }
    else
    {
        for (i = 0; i < M; i++)
        {
            lsf_r1[i] = efr_sub (lsf1[i], lsf_p_CN[i]);     move16 (); 
            lsf_r2[i] = efr_sub (lsf2[i], lsf_p_CN[i]);     move16 (); 
        }
    }
    ...
    if ((txdtx_ctrl & TX_SP_FLAG) != 0)
    {
        for (i = 0; i < M; i++)
        {
            lsf1_q[i] = efr_add (lsf_r1[i], lsf_p[i]);      move16 (); 
            lsf2_q[i] = efr_add (lsf_r2[i], lsf_p[i]);      move16 (); 
...
)
{
    ...
    {
        temp = efr_sub (lsf_r1[0], *p_dico++);
        temp = efr_mult (wf1[0], temp);
        dist = efr_L_mult (temp, temp);

        temp = efr_sub (lsf_r1[1], *p_dico++);
        temp = efr_mult (wf1[1], temp);
        dist = efr_L_mac (dist, temp, temp);

        temp = efr_sub (lsf_r2[0], *p_dico++);
        temp = efr_mult (wf2[0], temp);
        dist = efr_L_mac (dist, temp, temp);

        temp = efr_sub (lsf_r2[1], *p_dico++);
        temp = efr_mult (wf2[1], temp);
        dist = efr_L_mac (dist, temp, temp);

        test (); 
        if (efr_L_sub (dist, dist_min) < (Word32) 0)
        {
    ...
    p_dico = &dico[efr_shl (index, 2)];                     move16 (); 
    ...
    Word16 dico_size)   /* input : size of quantization codebook */
{
    ...
    for (i = 0; i < dico_size; i++)
    {
        ...
        p_dico -= 4;                                    move16 (); 
        temp = efr_add (lsf_r1[0], *p_dico++);
        temp = efr_mult (wf1[0], temp);
        dist = efr_L_mult (temp, temp);

        temp = efr_add (lsf_r1[1], *p_dico++);
        temp = efr_mult (wf1[1], temp);
        dist = efr_L_mac (dist, temp, temp);

        temp = efr_add (lsf_r2[0], *p_dico++);
        temp = efr_mult (wf2[0], temp);
        dist = efr_L_mac (dist, temp, temp);

        temp = efr_add (lsf_r2[1], *p_dico++);
        temp = efr_mult (wf2[1], temp);
        dist = efr_L_mac (dist, temp, temp);
    ...
    {
        lsf_r1[0] = efr_negate (*p_dico++);                 move16 (); 
        lsf_r1[1] = efr_negate (*p_dico++);                 move16 (); 
        lsf_r2[0] = efr_negate (*p_dico++);                 move16 (); 
        lsf_r2[1] = efr_negate (*p_dico++);                 move16 (); 
    }

    index = efr_shl (index, 1);
    index = efr_add (index, sign);
    ...
    Word16 *wf)          /* output: square of weighting factors */
{
    ...
    for (i = 1; i < 9; i++)
    {
        wf[i] = efr_sub (lsf[i + 1], lsf[i - 1]);           move16 (); 
    }
    /* wf[9] = 0.5 - lsf[8] */    
    wf[9] = efr_sub (16384, lsf[8]);move16 ();      

    for (i = 0; i < 10; i++)
    {
        temp = efr_sub (wf[i], 1843);
        test (); 
        if (temp < 0)
        {
            wf[i] = efr_sub (3427, efr_mult (wf[i], 28160));    move16 (); 
        }
        else
        {
            wf[i] = efr_sub (1843, efr_mult (temp, 6242));      move16 (); 
        }

        wf[i] = efr_shl (wf[i], 3); move16 (); 
    }
    return;
}
