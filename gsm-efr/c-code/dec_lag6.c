...
)
{
    ...
    if (pit_flag == 0)          /* if 1st or 3rd subframe */
    {
        ...
        {                       /* if bfi == 0 decode pitch */
            test (); 
            if (efr_sub (index, 463) < 0)
            {
                /* T0 = (index+5)/6 + 17 */
                T0 = efr_add (efr_mult (efr_add (index, 5), 5462), 17);
                i = efr_add (efr_add (T0, T0), T0);
                /* *T0_frac = index - T0*6 + 105 */
                *T0_frac = efr_add (efr_sub (index, efr_add (i, i)), 105);
                                move16 (); 
            }
            else
            {
                T0 = efr_sub (index, 368);
                *T0_frac = 0;   move16 (); 
            }
        }
        else
        ...
        /* second or fourth subframe */
    {
        test (); test (); 
        /* if bfi == 0 decode pitch */
        if ((bfi == 0) && (efr_sub (index, 61) < 0))
        {
            /* i = (index+5)/6 - 1 */
            i = efr_sub (efr_mult (efr_add (index, 5), 5462), 1);
            T0 = efr_add (i, T0_min);
            i = efr_add (efr_add (i, i), i);
            *T0_frac = efr_sub (efr_sub (index, 3), efr_add (i, i));
                                move16 (); 
        }
        else
            /* bfi == 1  OR index >= 61 */
