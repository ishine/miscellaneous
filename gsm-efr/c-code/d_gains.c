...
)
{
    ...
    if (bfi == 0)
    {
        ...
        {
            gain = efr_shr (qua_gain_pitch[index], 2);             move16 (); 

            test (); 
            if (prev_bf != 0)
            {
                test (); 
                if (efr_sub (gain, prev_gp) > 0)
                {
                    gain = prev_gp;
                }
            }
    ...
    else
    {
        ...
        if ((rxdtx_ctrl & RX_SP_FLAG) != 0)
        {
            ...
            test (); 
            if (efr_sub (tmp, past_gain_pit) < 0)
            {
                past_gain_pit = tmp;                           move16 (); 
            }
            gain = efr_mult (pdown[state], past_gain_pit);
        }
        else
        {
            gain = 0;                                          move16 (); 
    ...
    test (); 
    if (efr_sub (past_gain_pit, 4096) > 0)  /* if (past_gain_pit > 1.0) */
    {
...
)
{
        ...
        || ((rxdtx_ctrl & RX_LOST_SID_FRAME) != 0))
    {
        ...
        if ((rxdtx_ctrl & RX_DTX_MUTING) != 0)
        {
            ...
            gain_code_muting_CN = efr_mult (gain_code_muting_CN, 30057);
    ...
    if (bfi != 0)
    {
        ...
        test (); 
        if (efr_sub (tmp, past_gain_code) < 0)
        {
            past_gain_code = tmp;                              move16 (); 
        }
        past_gain_code = efr_mult (past_gain_code, cdown[state]);
        *gain_code = past_gain_code;                           move16 (); 

        av_pred_en = 0;                                        move16 (); 
        for (i = 0; i < 4; i++)
        {
            av_pred_en = efr_add (av_pred_en, past_qua_en[i]);
        }

        /* av_pred_en = 0.25*av_pred_en - 4/(20Log10(2)) */
        av_pred_en = efr_mult (av_pred_en, 8192);   /*  *= 0.25  */

        /* if (av_pred_en < -14/(20Log10(2))) av_pred_en = .. */
        test (); 
        if (efr_sub (av_pred_en, -2381) < 0)
        {
        ...
        test ();
        if (efr_sub (i_subfr, (3 * L_SUBFR)) == 0)
        {
    ...
    if ((rxdtx_ctrl & RX_SP_FLAG) != 0)
    {
        ...
        for (i = 0; i < lcode; i++)
        {
            ener_code = efr_L_mac (ener_code, code[i], code[i]);
        }
        /* ener_code = ener_code / lcode */
        ener_code = efr_L_mult (gsm_efr_round (ener_code), 26214);

        /* ener_code = 1/2 * Log2(ener_code) */
        Log2 (ener_code, &exp, &frac);
        ener_code = L_Comp (efr_sub (exp, 30), frac);
        ...
        for (i = 0; i < 4; i++)
        {
            ener = efr_L_mac (ener, past_qua_en[i], pred[i]);
        }
         ...
         *             = efr_Pow2(ener-ener_code)                                 *
         *                                           constant = 20*Log10(2)   *
         *-------------------------------------------------------------------*/

        ener = efr_L_shr (efr_L_sub (ener, ener_code), 1);
        L_Extract (ener, &exp, &frac);

        gcode0 = efr_extract_l (efr_Pow2 (exp, frac));  /* predicted gain */

        gcode0 = efr_shl (gcode0, 4);

        *gain_code = efr_mult (qua_gain_code[index], gcode0);      move16 (); 

        test (); 
        if (prev_bf != 0)
        {
            test (); 
            if (efr_sub (*gain_code, prev_gc) > 0)
            {
                *gain_code = prev_gc;     move16 (); 
            }
        }
        ...
        Log2 (efr_L_deposit_l (qua_gain_code[index]), &exp, &frac);

        past_qua_en[0] = efr_shr (frac, 5);                        move16 (); 
        past_qua_en[0] = efr_add (past_qua_en[0], efr_shl (efr_sub (exp, 11), 10));
        move16 (); 
    ...
    else
    {
        ...
        {
            gain_code_new_CN = efr_mult (gcode0_CN, qua_gain_code[index]);
