 ...
 *
 *   PURPOSE: DTX handler of the speech encoder. Determines when to efr_add
...
void tx_dtx (
  ...
  txdtx_N_elapsed = efr_add (txdtx_N_elapsed, 1);

  /* If voice activity was detected, reset hangover counter */

  test ();
  if (efr_sub (VAD_flag, 1) == 0) {
    ...
    *txdtx_ctrl = TX_SP_FLAG | TX_VAD_FLAG; move16 (); logic16 ();
    ...
    if (txdtx_hangover == 0) {
      ...
      txdtx_hangover = efr_sub (txdtx_hangover, 1);
      ...
      test ();
      if (efr_sub (efr_add (txdtx_N_elapsed, txdtx_hangover),
...
void rx_dtx (
  ...
  if ((*rxdtx_ctrl & RX_SP_FLAG) != 0) {
    test (); test (); test (); test ();
    if (efr_sub (frame_type, VALID_SID_FRAME) == 0) {
      *rxdtx_ctrl = RX_FIRST_SID_UPDATE;          move16 ();
    } else if (efr_sub (frame_type, INVALID_SID_FRAME) == 0) {
      *rxdtx_ctrl = RX_FIRST_SID_UPDATE
                    | RX_INVALID_SID_FRAME; move16 (); logic16();
    } else if (efr_sub (frame_type, UNUSABLE_FRAME) == 0) {
      *rxdtx_ctrl = RX_SP_FLAG;                   move16 ();
    } else if (efr_sub (frame_type, GOOD_SPEECH_FRAME) == 0) {
      *rxdtx_ctrl = RX_SP_FLAG;                   move16 ();
    }
  } else {
    test (); test (); test (); test ();
  ...
  if ((*rxdtx_ctrl & RX_SP_FLAG) != 0) {
    ...
    if ((*rxdtx_ctrl & RX_CONT_SID_UPDATE) != 0) {
      ...
      if (efr_sub (frame_type, VALID_SID_FRAME) == 0) {
        ...
        test ();
        if (efr_sub(rx_dtx_state, (CN_INT_PERIOD - 1)) < 0) {
          rx_dtx_state = efr_add(rx_dtx_state, 1); move16 ();
        }
      }
    }
    ...
    if ((*rxdtx_ctrl & RX_CNI_BFI) != 0) {
      ...
      if (efr_sub (TAF, 1) == 0) {
        *rxdtx_ctrl = *rxdtx_ctrl | RX_LOST_SID_FRAME;
        move16 ();  logic16 ();
        prev_SID_frames_lost = efr_add (prev_SID_frames_lost, 1);
      ...
      test ();
      if (efr_sub (prev_SID_frames_lost, 1) > 0) {
  ...
  rxdtx_N_elapsed = efr_add (rxdtx_N_elapsed, 1);
  ...
  if ((*rxdtx_ctrl & RX_SP_FLAG) != 0) {
    ...
    test (); test ();
    if (efr_sub (rxdtx_N_elapsed, DTX_ELAPSED_THRESHOLD) > 0) {
      *rxdtx_ctrl |= RX_UPD_SID_QUANT_MEM;        move16 (); logic16 ();
      ...
      rxdtx_aver_period = efr_sub (rxdtx_aver_period, 1);
    }
  }
...
Word16 sid_frame_detection (
  ...
  for (i = 0; i < 95; i++) {
    ...
    if (ser2[SID_codeword_bit_idx[i]] == 0) {
      nbr_errors = efr_add (nbr_errors, 1);
    }
  }

  /* Frame classification */
  test (); test ();
  if (efr_sub (nbr_errors, VALID_SID_THRESH) < 0) {
    /* Valid SID frame */
    sid = 2;                                        move16 ();
  } else if (efr_sub (nbr_errors, INVALID_SID_THRESH) < 0) {
    /* Invalid SID frame */
    sid = 1;                                        move16 ();
  } else {
    /* Speech frame */
...
void update_lsf_history (
  ...
  for (i = 0; i < M; i++) {
    temp = efr_add (efr_shr (lsf1[i], 1), efr_shr (lsf2[i], 1));
...
void update_lsf_p_CN (
  ...
  for (j = 0; j < M; j++) {
    L_temp = efr_L_mult (INV_DTX_HANGOVER, lsf_old[0][j]);
    for (i = 1; i < DTX_HANGOVER; i++) {
      L_temp = efr_L_mac (L_temp, INV_DTX_HANGOVER, lsf_old[i][j]);
    }
    lsf_p_CN[j] = gsm_efr_round (L_temp);                  move16 ();
  }
...
void aver_lsf_history (
  ...
  for (j = 0; j < M; j++) {
    L_temp = efr_L_mult (0x3fff, lsf1[j]);
    L_temp = efr_L_mac (L_temp, 0x3fff, lsf2[j]);
    L_temp = efr_L_mult (INV_DTX_HANGOVER_P1, efr_extract_h (L_temp));

    for (i = 0; i < DTX_HANGOVER; i++) {
      L_temp = efr_L_mac (L_temp, INV_DTX_HANGOVER_P1, lsf_old[i][j]);
    }

    lsf_aver[j] = efr_extract_h (L_temp);              move16 ();
  }
...
void update_gain_code_history_tx (
  ...
  test ();
  if (efr_sub (buf_p_tx, (4 * DTX_HANGOVER - 1)) == 0) {
    buf_p_tx = 0;                                  move16 ();
  } else {
    buf_p_tx = efr_add (buf_p_tx, 1);
  }
...
void update_gain_code_history_rx (
  ...
  test ();
  if (efr_sub (buf_p_rx, (4 * DTX_HANGOVER - 1)) == 0) {
    buf_p_rx = 0;                                  move16 ();
  } else {
    buf_p_rx = efr_add (buf_p_rx, 1);
  }
...
Word16 compute_CN_excitation_gain (
  ...
  do {
    ...
    L_temp = 0L;                                   move32 ();
    for (i = 0; i < L_SUBFR; i++) {
      temp = efr_shr (res2[i], norm);
      L_temp = efr_L_mac (L_temp, temp, temp);
    }

    test ();
    if (efr_L_sub (L_temp, MAX_32) == 0) {
      norm = efr_add (norm, 1);
  ...
  L_temp = efr_L_add (L_temp, 1L);             /* Avoid the case of all zeros */

  /* Take the square root of the obtained energy value (sqroot is a 2nd
     order Taylor series approximation) */

  norm1 = efr_norm_l (L_temp);
  temp = efr_extract_h (efr_L_shl (L_temp, norm1));
  L_temp = efr_L_mult (temp, temp);
  L_temp = efr_L_sub (805306368L, efr_L_shr (L_temp, 3));
  L_temp = efr_L_add (L_temp, efr_L_mult (24576, temp));

  temp = efr_extract_h (L_temp);
  test (); logic16 ();
  if ((norm1 & 0x0001) != 0) {
    temp = efr_mult_r (temp, 23170);
    norm1 = efr_sub (norm1, 1);
  }
  /* Divide the result of sqroot operation by sqroot(10) */

  temp = efr_mult_r (temp, 10362);

  /* Re-scale to get the final value */

  norm1 = efr_shr (norm1, 1);
  norm1 = efr_sub (norm1, norm);

  test ();
  if (norm1 >= 0) {
    temp = efr_shr (temp, norm1);
  } else {
    temp = efr_shl (temp, efr_abs_s (norm1));
  }
...
Word16 update_gcode0_CN (
  ...
  L_ret = 0L;                                        move32 ();
  for (i = 0; i < DTX_HANGOVER; i++) {
    L_temp = efr_L_mult (0x1fff, gain_code_old[4 * i]);
    for (j = 1; j < 4; j++) {
      L_temp = efr_L_mac (L_temp, 0x1fff, gain_code_old[4 * i + j]);
    }
    L_ret = efr_L_mac (L_ret, INV_DTX_HANGOVER, efr_extract_h (L_temp));
  }

  return efr_extract_h (L_ret);
}
...
Word16 aver_gain_code_history (
  ...
  L_ret = efr_L_mult (0x470, CN_excitation_gain);

  for (i = 0; i < (4 * DTX_HANGOVER); i++) {
    L_ret = efr_L_mac (L_ret, 0x470, gain_code_old[i]);
  }
  return efr_extract_h (L_ret);
}
...
void build_CN_code (
  ...
  for (k = 0; k < NB_PULSE; k++) {
    ...
    i = efr_shr (efr_extract_l (efr_L_mult (i, 10)), 1);
    i = efr_add (i, k);
...
Word16 pseudonoise (
  ...
  for (i = 0; i < no_bits; i++) {
    ...
    noise_bits = efr_shl (noise_bits, 1);
    noise_bits = noise_bits | (efr_extract_l (*shift_reg) & 1);
    logic16 (); logic16 ();

    *shift_reg = efr_L_shr (*shift_reg, 1);            move32 ();
    test (); logic16 ();
    if (Sn & 1) {
...
Word16 interpolate_CN_param (
  ...
  L_temp = efr_L_mult (interp_factor[rx_dtx_state], new_param);
  temp = efr_sub (0x7fff, interp_factor[rx_dtx_state]);
  temp = efr_add (temp, 1);
  L_temp = efr_L_mac (L_temp, temp, old_param);
  temp = gsm_efr_round (L_temp);
