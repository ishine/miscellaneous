...
)
{
    ...
    for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
    {
        ...
        L_tmp = efr_L_mult (h[0], h[0]);
        for (i = 1; i < L_H; i++)
        {
            L_tmp = efr_L_mac (L_tmp, h[i], h[i]);
        }
        temp1 = efr_extract_h (L_tmp);

        L_tmp = efr_L_mult (h[0], h[1]);
        for (i = 1; i < L_H - 1; i++)
        {
            L_tmp = efr_L_mac (L_tmp, h[i], h[i + 1]);
        }
        temp2 = efr_extract_h (L_tmp);
        ...
        {
            temp2 = efr_mult (temp2, MU);
            temp2 = efr_div_s (temp2, temp1);
        }
