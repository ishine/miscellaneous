...
)
{
    ...
    for (i = 0; i < m; i++)
    {
        ind = efr_shr (lsf[i], 8);      /* ind    = b8-b15 of lsf[i] */
        offset = lsf[i] & 0x00ff;   logic16 (); /* offset = b0-b7  of lsf[i] */

        /* lsp[i] = table[ind]+ ((table[ind+1]-table[ind])*offset) / 256 */

        L_tmp = efr_L_mult (efr_sub (table[ind + 1], table[ind]), offset);
        lsp[i] = efr_add (table[ind], efr_extract_l (efr_L_shr (L_tmp, 9)));
                                    move16 (); 
    }
    return;
}
...
)
{
    ...
    for (i = m - 1; i >= 0; i--)
    {
        ...
        test (); 
        while (efr_sub (table[ind], lsp[i]) < 0)
        {
            ind--;
            test (); 
        }
        ...
        L_tmp = efr_L_mult (efr_sub (lsp[i], table[ind]), slope[ind]);
        /*(lsp[i]-table[ind])*slope[ind])>>12*/
        lsf[i] = gsm_efr_round (efr_L_shl (L_tmp, 3));      move16 (); 
        lsf[i] = efr_add (lsf[i], efr_shl (ind, 8));    move16 (); 
    }
    return;
}
