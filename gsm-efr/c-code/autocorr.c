...
Word16 Autocorr (
    ...
    for (i = 0; i < L_WINDOW; i++) {
        y[i] = efr_mult_r (x[i], wind[i]); move16 ();
    }
    ...
    do {
        ...
        for (i = 0; i < L_WINDOW; i++) {
            sum = efr_L_mac (sum, y[i], y[i]);
        }

        /* If overflow divide y[] by 4 */

        test ();
        if (efr_L_sub (sum, MAX_32) == 0L) {
            overfl_shft = efr_add (overfl_shft, 4);
            overfl = 1;                move16 (); /* Set the overflow flag */

            for (i = 0; i < L_WINDOW; i++) {
                y[i] = efr_shr (y[i], 2);  move16 ();
            }
        }
        test ();
    } while (overfl != 0);

    sum = efr_L_add (sum, 1L);             /* Avoid the case of all zeros */

    /* Normalization of r[0] */

    norm = efr_norm_l (sum);
    sum = efr_L_shl (sum, norm);
