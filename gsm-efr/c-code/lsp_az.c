...
)
{
    ...
    for (i = 5; i > 0; i--)
    {
        f1[i] = efr_L_add (f1[i], f1[i - 1]);    move32 (); /* f1[i] += f1[i-1]; */
        f2[i] = efr_L_sub (f2[i], f2[i - 1]);    move32 (); /* f2[i] -= f2[i-1]; */
    }

    a[0] = 4096;                             move16 (); 
    for (i = 1, j = 10; i <= 5; i++, j--)
    {
        t0 = efr_L_add (f1[i], f2[i]);           /* f1[i] + f2[i] */
        a[i] = efr_extract_l (efr_L_shr_r (t0, 13)); move16 (); 
        t0 = efr_L_sub (f1[i], f2[i]);           /* f1[i] - f2[i] */
        a[j] = efr_extract_l (efr_L_shr_r (t0, 13)); move16 (); 
    }
...
static void Get_lsp_pol (Word16 *lsp, Word32 *f)
{
    ...
    /* f[0] = 1.0;             */
    *f = efr_L_mult (4096, 2048);              move32 (); 
    f++;                                   move32 (); 
    *f = efr_L_msu ((Word32) 0, *lsp, 512);    /* f[1] =  -2.0 * lsp[0];  */
    f++;                                   move32 (); 
    ...
    for (i = 2; i <= 5; i++)
    {
        *f = f[-2];                        move32 (); 
        ...
        {
            L_Extract (f[-1], &hi, &lo);
            t0 = efr_Mpy_32_16 (hi, lo, *lsp); /* t0 = f[-1] * lsp    */
            t0 = efr_L_shl (t0, 1);
            *f = efr_L_add (*f, f[-2]);        move32 (); /* *f += f[-2]      */
            *f = efr_L_sub (*f, t0);move32 (); /* *f -= t0            */
        }
        *f = efr_L_msu (*f, *lsp, 512);        move32 (); /* *f -= lsp<<9     */
