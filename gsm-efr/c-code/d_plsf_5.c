...
)
{
    ...
    if (bfi != 0)                               /* if bad frame */
    {
        ...
        for (i = 0; i < M; i++)
        {
            ...
            lsf1_q[i] = efr_add (efr_mult (past_lsf_q[i], ALPHA),
                             efr_mult (mean_lsf[i], ONE_ALPHA));
                                                move16 (); 
        ...
        for (i = 0; i < M; i++)
        {
            ...
            temp = efr_add (mean_lsf[i], efr_mult (past_r2_q[i], PRED_FAC));

            past_r2_q[i] = efr_sub (lsf2_q[i], temp);
                                                move16 (); 
        }
    }
    else
        /* if good LSFs received */
    {
        ...
        i = efr_shr (indice[2], 1);
        p_dico = &dico3_lsf[efr_shl (i, 2)];        move16 (); 
        ...
        {
            lsf1_r[4] = efr_negate (*p_dico++);     move16 (); 
            lsf1_r[5] = efr_negate (*p_dico++);     move16 (); 
            lsf2_r[4] = efr_negate (*p_dico++);     move16 (); 
            lsf2_r[5] = efr_negate (*p_dico++);     move16 (); 
        }

        p_dico = &dico4_lsf[efr_shl (indice[3], 2)];move16 (); 
        lsf1_r[6] = *p_dico++;                  move16 (); 
        lsf1_r[7] = *p_dico++;                  move16 (); 
        lsf2_r[6] = *p_dico++;                  move16 (); 
        lsf2_r[7] = *p_dico++;                  move16 (); 

        p_dico = &dico5_lsf[efr_shl (indice[4], 2)];move16 (); 
        ...
        if ((rxdtx_ctrl & RX_SP_FLAG) != 0)
        {
            ...
            {
                temp = efr_add (mean_lsf[i], efr_mult (past_r2_q[i], PRED_FAC));
                lsf1_q[i] = efr_add (lsf1_r[i], temp);
                                                move16 (); 
                lsf2_q[i] = efr_add (lsf2_r[i], temp);
                                                move16 (); 
        ...
        {                       /* Valid SID frame */
            for (i = 0; i < M; i++)
            {
                lsf2_q[i] = efr_add (lsf2_r[i], lsf_p_CN[i]);
                                                move16 (); 
