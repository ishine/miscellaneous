...
)
{
    ...
    if (pit_flag == 0)          /* if 1st or 3rd subframe */
    {
        ...
        if (efr_sub (T0, 94) <= 0)
        {
            ...
            index = efr_add (efr_sub (efr_add (i, i), 105), *T0_frac);
        } else
        {   /* set fraction to 0 for delays > 94 */
            *T0_frac = 0;       move16 (); 
            index = efr_add (T0, 368);
        }

        /* find T0_min and T0_max for second (or fourth) subframe */

        *T0_min = efr_sub (T0, 5);  move16 (); 
        test (); 
        ...
        *T0_max = efr_add (*T0_min, 9);
        ...
        if (efr_sub (*T0_max, pit_max) > 0)
        {
            ...
            *T0_min = efr_sub (*T0_max, 9); move16 (); 
        }
    } else
        /* if second or fourth subframe */
    {
        ...
        index = efr_add (efr_add (efr_add (i, i), 3), *T0_frac);
    }
